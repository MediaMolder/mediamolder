...
Processors may also implement the optional `FrameSource` sub-interface to **generate their own frames** rather than processing inbound ones.  Two built-in FrameSources assemble clip timelines — **`sequence_editor`** (multi-track NLE timeline with placement, layering, the full libavfilter `xfade` transition set, and **audio mixing with auto-coupled crossfades**) and **`xfade_sequence`** (single-track linear montage, O(1-frame) memory) — so you can cut, trim, and add wipes/dissolves/slides (and matching audio crossfades) entirely inside a job graph.  See the [Video Editing Guide](docs/video-editing-guide.md) for the workflow and timing model, or the [timeline-assembly comparison](docs/go-processor-nodes.md#timeline-assembly-sequence_editor-vs-xfade_sequence) for which to use.
