...
- [Remote Server (`mediamolder serve` + `job`)](docs/remote-server.md) — Tier 1 (`--mode=server`) and Tier 2 distributed (`--mode=api`, `--mode=worker`, `schema_version "1.4"` Job documents, `single`/`fanout_static` strategies, Postgres + NATS/SQS, leaderless reconciler, DLQ)
