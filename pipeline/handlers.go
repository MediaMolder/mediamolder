...
func (r *graphRunner) createEncoder(dag *graph.Graph, node *graph.Node) (*av.EncoderContext, error) {
		...
		for _, e := range node.Outbound {
			if e.To.Kind == graph.KindSink {
				...
				if out != nil {
					...
					} else if e.Type == graph.PortAudio {
	...
	edgeType := node.Inbound[0].Type
	if edgeType == graph.PortVideo {
	...
	} else if edgeType == graph.PortAudio {
...
func (r *graphRunner) openSink(dag *graph.Graph, node *graph.Node) (*sinkResources, error) {
...
func (r *graphRunner) upstreamFilterGraph(dag *graph.Graph, node *graph.Node) *av.FilterGraph {
	if len(node.Inbound) == 0 {
		return nil
	}
